...
public class ModifyDrinks extends javax.swing.JFrame {
    ...
    this.loggedInUsername = loggedInUsername; // Store the loggedInUsername
        ...
        setTitle("Vending Machine System - Modify Drinks"); // Set the title of the frame
    ...
    private void initComponents() {
        ...
        List<String> drinkOptions = getDrinkOptions();

        // Add the initial message as the first option
        String initialMessage = "Please select the drink to modify";
        JMenuItem initialMenuItem = new JMenuItem(initialMessage);
        initialMenuItem.setEnabled(false); // Disable selection of this item
        popupMenu.add(initialMenuItem);

        for (String option : drinkOptions) {
            JMenuItem menuItem = new JMenuItem(option);
            menuItem.addActionListener(new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent e) {
                    jTextField1.setText(option);
                    popupMenu.setVisible(false);
                    
                     // Trigger the handleMenuItemClick method here
                    handleMenuItemClick(option); // Pass the selected option as an argument
                }
            });
        ...
        jTextField1.setEditable(false); // Make the field non-editable
        
        // Disable key input events for jTextField1
        jTextField1.addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                e.consume(); // Consume the key event to prevent input
            }
        });
        ...
        jTextField1.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getButton() == MouseEvent.BUTTON1) {
                    jTextField1.setText(""); // Clear the hint text when clicked
                    popupMenu.show(jTextField1, 0, jTextField1.getHeight()); // Display the popup menu
                }
            }
        });;
        ...
        popupMenu.addPopupMenuListener(new PopupMenuListener() {
            ...
            public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
                // Not needed, but can be left empty
            }

            @Override
            public void popupMenuWillBecomeInvisible(PopupMenuEvent e) {
                // Hide the popup menu after a short delay to avoid flickering
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (emptyFieldCount > 1) {
                JOptionPane.showMessageDialog(ModifyDrinks.this, "Please don't leave the inputs blank !", "Input Error", JOptionPane.ERROR_MESSAGE);
            ...
            if (!errorMessage.isEmpty()) {
                JOptionPane.showMessageDialog(ModifyDrinks.this, errorMessage, "Input Error", JOptionPane.ERROR_MESSAGE);
            ...
            if (drinkExists(drinkName)) {
                JOptionPane.showMessageDialog(ModifyDrinks.this, "Can't add. Drink already exists !");
                return;
            }
            ...
            if (confirmResult == JOptionPane.YES_OPTION) {
                String selectedOption = jTextField1.getText(); // Get the value from jTextField1
                String drinkID = selectedOption.substring(selectedOption.lastIndexOf("(") + 1, selectedOption.lastIndexOf(")"));
                ...
                try (BufferedReader reader = new BufferedReader(new FileReader("data/drinks.txt"))) {
                    ...
                    while ((line = reader.readLine()) != null) {
                        ...
                        if (parts.length >= 1 && parts[0].equals(drinkID)) {
                            // Update the price from jTextField3
                            double price = Double.parseDouble(jTextField3.getText());
                            DecimalFormat decimalFormat = new DecimalFormat("0.00");
                            String formattedPrice = decimalFormat.format(price);

                            // Replace the portions with updated values
                            parts[2] = formattedPrice;
                            ...
                            lines.add(line); // Keep other lines as they are
                        }
                    }
                ...
                if (fileName != null && !fileName.isEmpty()) {
                    ...
                    // Delete the old image file with the same name
                    String oldResourcePath = "/resources/" + oldFileName;
                    URL oldResourceUrl = getClass().getResource(oldResourcePath);
                    if (oldResourceUrl != null) {
                        File oldResourceFile = new File(oldResourceUrl.getPath());
                        if (oldResourceFile.exists()) {
                            oldResourceFile.delete();
                        }
                    }

                    // Get the URL to the resources folder
                    URL resourcesUrl = getClass().getResource("/resources");
                    if (resourcesUrl == null) {
                        JOptionPane.showMessageDialog(ModifyDrinks.this, "Error accessing resources folder.");
                        return; // Exit the method if accessing resources folder failed
                    }

                    File resourcesFolder;
                    try {
                        resourcesFolder = new File(resourcesUrl.toURI());
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        JOptionPane.showMessageDialog(ModifyDrinks.this, "Error saving file to resources folder.");
                        return; // Exit the method if file saving failed
                    }
                }

                
                if (updateRecord(lines)) {
                    int confirmContinue = JOptionPane.showConfirmDialog(
                        ModifyDrinks.this,
                        "Drink updated successfully! Do you want to continue updating drinks?",
                        "Update Another Drink",
                        JOptionPane.YES_NO_OPTION
                    );

                    if (confirmContinue == JOptionPane.YES_OPTION) {
                        // Clear the input fields and reset values
                        jTextField1.setText("Please select the drink to modify");
                        setHint(jTextField2, "Please enter the quantity");
                        setHint(jTextField3, "Please enter the unit price (RM)");
                        ...
                        // Reset selectedFileNameLabel
                        selectedFileNameLabel.setText("No file selected");
                    } else {
                        // Close the ModifyDrinks frame
                        dispose();
                        openViewDrinksFrame(loggedInUsername);
                    }
                } else {
                    JOptionPane.showMessageDialog(ModifyDrinks.this, "Failed to update the drink.");
                }
            } catch (IOException e) {
                e.printStackTrace();
                JOptionPane.showMessageDialog(ModifyDrinks.this, "Error updating data.");
            }
        }
        }
    });
...
layout.setVerticalGroup(
    ...
    private void initializeTextField1() {
        int textFieldWidth = 400; // Set the desired width for the text fields
        int textFieldHeight = 40; // Set the desired height for the text fields
        int padding = 10; // Set the desired padding value

        // Set the initial message as hint text for the jTextField1
        jTextField1.setForeground(new ColorUIResource(HINT_TEXT_COLOR));
        jTextField1.setBackground(Color.WHITE); // Set background color
        jTextField1.setBorder(BorderFactory.createLineBorder(Color.GRAY)); // Add black border
        jTextField1.setText("Please select the drink to modify");

        // Add the initial message as the first option
        JMenuItem initialMenuItem = new JMenuItem("Please select the drink to modify");
        initialMenuItem.setEnabled(false); // Disable selection of this item
        popupMenu.add(initialMenuItem);

        // Set preferred size and add padding to jTextField1
        jTextField1.setPreferredSize(new Dimension(textFieldWidth, textFieldHeight));
        addPadding(jTextField1, padding);
    }
    
    private void handleMenuItemClick(String selectedOption) {
    String selectedDrinkId = selectedOption.substring(selectedOption.lastIndexOf("(") + 1, selectedOption.lastIndexOf(")"));
    System.out.println("Selected Drink ID: " + selectedDrinkId); // Print the selected drink ID

    try (BufferedReader reader = new BufferedReader(new FileReader("data/drinks.txt"))) {
        String line;
        while ((line = reader.readLine()) != null) {
            String[] parts = line.split("\\|");
            if (parts.length >= 1 && parts[0].equals(selectedDrinkId)) {
                jTextField2.setText(parts[3]); // Set portion 3 to jTextField2
                jTextField3.setText(parts[2]); // Set portion 4 to jTextField3
                filePath = parts[4]; // Set portion 5 to filePath
                
                // Extract the filename from the filePath
                int lastSlashIndex = filePath.lastIndexOf('/');
                String selectedFileName = lastSlashIndex != -1 ? filePath.substring(lastSlashIndex + 1) : filePath;
                
                String oldFileName = selectedFileName; // Save the value as oldFileName
                selectedFileNameLabel.setText(selectedFileName); // Set the extracted filename to selectedFileNameLabel

                break; // Exit the loop since we found the record                
            }
        }
    } catch (IOException e) {
        e.printStackTrace();
    ...
    private boolean updateRecord(List<String> updatedLines) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("data/drinks.txt"))) {
    ...
    private List<String> getDrinkOptions() {
